#Script for drawing three numLQ histograms (on the same canvas) for three different mass and coupling points
...
for j, label in enumerate(labels):
    ...
    histos[label] = plotdir.Get('numLQ;1')

    print('Got the histogram')

    histos[label].SetNdivisions(505)
    ...
    histos[label].SetTitle('Number of Leptoquarks, #lambda = 1.0')

    numEvents = 0
    for i in range(histos[label].GetNbinsX()):
	numEvents += histos[label].GetBinContent(i+1)

    histos[label].SetMaximum(numEvents)
...
legend.Draw()
canv.Print("savedPlots/comparisonPlots/numLQPlot_mass.png")
...
for j, label in enumerate(labels):
    ...
    histos[label].GetYaxis().SetTitle('Number of Events (Normalized)')
    histos[label].SetLineColor(colors[j])
    histos[label].SetTitle('Number of Leptoquarks, M_{LQ} = 1.4 TeV')    
...
legend2.Draw()
canv2.Print("savedPlots/comparisonPlots/numLQPlot_coupling.png")
